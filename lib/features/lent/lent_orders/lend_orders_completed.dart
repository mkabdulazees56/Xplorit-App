...
class LentOrdersCompleted extends StatefulWidget {
  const LentOrdersCompleted({required this.garageId, Key? key})
      : super(key: key);

  final String garageId;
  @override
  State<LentOrdersCompleted> createState() => _LentOrdersCompleted();
}

class _LentOrdersCompleted extends State<LentOrdersCompleted> {
  int bottomNavBarCurrentIndex = 0;
  String renterImage = TImages.profile;
  String renterLocation = '';
  double renterLatitude = 0.0;
  double renterLongitude = 0.0;
  double rating = 0.0;
  String model = '';
  int noOfRating = 0;
  int noOfDays = 0;
...
// Asynchronous method to fetch vehicle data
  ...
  Future<void> _fetchAddressName(double lat, double long) async {
    try {
      ...
      renterLocation = 'Error fetching address';
    }
  }
    ...
    for (var completedRent in completedRents) {
      ...
      for (RenterModel renter in renterController.rentersData) {
        if (renter.renterId == completedRent.renterId) {
          ...
          renterLatitude = renter.latitude;
          renterLongitude = renter.longitude;
          await _fetchAddressName(renterLatitude, renterLongitude);
        }
      }

      vehicleWidget = VehicleRentalWidget(
          lenderImg: renterImage,
          lender: renterName,
          lenderLocation: renterLocation,
